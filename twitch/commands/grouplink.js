        ...
        con.query("select id from group__user join `group` on `group`.id = group__user.group_id where group__user.user_id = ? and `group`.active order by `group`.starttime asc;", [streamer.id], (err, res) => {
            if (!err) {
                if (res.length === 0) {
                    ...
                    global.client.ban.say(streamer.display_name.toLowerCase(), `Visit ${config.pub_domain.replace("https://", "").replace("http://", "")}g/u/${encodeURI(streamer.display_name.toLowerCase())} to view the active group!`)
                }
            } else {
                handleError(err);
            }
